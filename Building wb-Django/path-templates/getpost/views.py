...
class AwesomeView(View) : #It will separate the Noptification page and the form page.  "FLASH MESSAGE PATTERN"
    def get(self, request):
        msg = request.session.get('msg', False)
        if ( msg ) : del(request.session['msg']) #deletes the mesagge when reloading it with no response
        return render(request, 'getpost/guess.html', {'message' : msg })
    ...
    def post(self, request):
        ...
        return redirect(request.path) #redirects it to the get function and run the page.  /gpost/awesome.html
